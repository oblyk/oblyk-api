...
module Api
  module V1
    class UsersController < ApiController
      ...
      before_action :protected_outdoor_log_book, only: %i[ascended_crag_routes]
      ...
      def search
        ...
        hits = User.includes(avatar_attachment: :blob, banner_attachment: :blob)
                   .search(query, page: page, hits_per_page: per_page)

        serializer = serializer(
          UserSerializer,
          hits,
          {
            params: { include_attachments: { User: %i[avatar banner] } },
            meta: {
              query: query,
