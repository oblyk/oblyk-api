...
module ContestService
  class RankerTest < ActiveSupport::TestCase
    ...
    test 'scores returns division point' do
      ...
      # Dans les fixtures, il y a probablement une seule ascension pour cette route dans cette catégorie/genre
      # point = 1000 / count_by_routes[route_id]
    ...
    test 'scores for ZONE_AND_TOP_REALISED ranking type' do
      ...
      ranker = ContestService::Ranker.new(@step, @category, @genre)
      ascent = contest_participant_ascents(:ascent_2) # top_attempt: 2, zone_1_attempt: 1
      
      score = ranker.scores(ascent)
      assert_equal 1.001, score[:value] # value = 1.001 if top
